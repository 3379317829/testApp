...
(function (global) {
  ...
  var STATUS = {
    ...
    open: { key: 'open', label: '报名 / 参与中', tone: 'open', rank: 3 },
    waitlist: { key: 'waitlist', label: '已截止 · 可候补', tone: 'soft', rank: 4 },
    ...
    for (var i = 0; i < c.starts.length; i++) {
      ...
      if (s <= now && now <= end) {
        return Object.assign({}, STATUS.ongoing, {
          detail: '进行中，' + (c.ends.length ? '预计 ' + fmt(c.ends[c.ends.length - 1].t.at) + ' 结束' : '持续时间未提供')
        });
      }
    }
    ...
    if (c.deadlines.length) {
      ...
      if (item.signup && item.signup.unsure && futureStart) {
        return Object.assign({}, STATUS.waitlist, {
          detail: '报名已于 ' + fmt(c.deadlines[0].t.at) + ' 截止，' + (item.requirement.indexOf('候补') > -1 ? '可到现场尝试候补' : '仍可关注后续通知')
        });
      }
      if (futureStart) {
  ...
  function scoreOf(item) {
    ...
    var level, label;
    if (score >= 85) { level = 'high'; label = '信息较完整'; }
    else if (score >= 60) { level = 'mid'; label = '信息基本可用'; }
    else if (score >= 40) { level = 'low'; label = '信息有限'; }
    else { level = 'risk'; label = '请谨慎参考'; }

    /* 待确认项：场地待定、报名待审核、时间模糊等 */
    var pending = [];
    if (item.place && item.place.status === 'pending') pending.push('地点待确认');
    if (item.signup && item.signup.unsure) pending.push('报名结果待审核/待确认');
